...
/**
 * The enum constants in CompatSound are based on the 1.15.2 Sound enum.
...
public enum CompatSound {
	...
	private CompatSound(String legacy) {
		this.legacy = legacy;
	}
	...
	public boolean exists() {
		if (this.sound == null) {
			try {
				this.sound = Sound.valueOf(this.toString());
			} catch (Exception e) {}
			
			try {
				this.sound = Sound.valueOf(this.legacy);
			} catch (Exception e) {}	
		}
		return sound != null;
	}
	...
	public Sound asSound() {
		...
		try {
			return this.sound = Sound.valueOf(this.toString());
		} catch (Exception e) {}
		
		try {
			return this.sound = Sound.valueOf(this.legacy);
		} catch (Exception e) {}
		
		Console.warn("A Sound requested was not found! Please contact the developer, he needs to update the plugin! ");
		Console.warn("Sound=(" + this.toString() + ") | Version=(" + Reflect.nmsVersion() + ")");
		return null;
	}
	
	public void play(UserCore user, Location loc, float distance, float pitch) {
		user.getPlayer().playSound(loc, this.asSound(), distance, pitch);
	}
	
	public void play(UserCore user, float distance, float pitch) {
		user.getPlayer().playSound(user.getLocation(), this.asSound(), distance, pitch);
	}
	
	public void play(Location loc, float distance, float pitch) {
		loc.getWorld().playSound(loc, this.asSound(), distance, pitch);
	}
	
	public void stop(UserCore user) {
		user.getPlayer().stopSound(this.asSound());
	}
}
